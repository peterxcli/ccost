...
pub(crate) fn parse_error_session(path: &Path, err: anyhow::Error) -> Session {
    Session {
        id: path
            .file_stem()
            .and_then(|s| s.to_str())
...
pub(crate) fn parse_session_with_fingerprint_or_error(
    _root: &Path,
    path: &Path,
    relative_path: &str,
...
pub(crate) fn parse_session_with_fingerprint(
    path: &Path,
    relative_path: &str,
    metadata: FileMetadataParts,
) -> Result<ParsedSessionFile> {
...
pub(crate) fn parse_session_inner(
    path: &Path,
...
pub(crate) fn extract_message_text(payload: &Value) -> String {
...
pub(crate) fn is_searchable_user_message(text: &str) -> bool {
...
pub(crate) fn model_from_payload(value: Option<&Value>) -> Option<String> {
    let value = value?;
    ["model", "model_name"]
        .into_iter()
...
pub(crate) fn model_from_result(value: &Value) -> Option<String> {
...
pub(crate) fn usage_from_token_count(info: &Value) -> Option<TokenUsage> {
...
pub(crate) fn usage_object_from_result(value: &Value) -> Option<&Value> {
    value
        .get("usage")
...
pub(crate) fn usage_from_exec_result(value: &Value) -> Option<TokenUsage> {
...
pub(crate) fn timestamp_from_result(value: &Value) -> Option<String> {
...
pub(crate) fn timestamp_value(value: Option<&Value>) -> Option<String> {
    match value? {
        Value::String(text) => {
            let text = text.trim();
            (!text.is_empty()).then(|| text.to_string())
...
pub(crate) fn infer_id_from_path(path: &Path) -> String {
    let stem = path
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("unknown");
